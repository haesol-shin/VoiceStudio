...
class SIMCalculator(BaseMetricCalculator):
    ...
    def extract_embeddings(self, paths: list[Path]) -> dict[Path, torch.Tensor]:
        """Extract normalized speaker embeddings for a list of audio paths.

        Reuses the batched loading logic from ``calculate_batch_optimized``.
        Embeddings are returned on CPU, L2-normalized.

        Args:
            paths: Audio file paths (duplicates are deduplicated internally).

        Returns:
            Dict mapping each successfully processed path → embedding tensor.
        """
        ...
        for i in tqdm(
            range(0, len(unique_paths), batch_size),
            ...
            valid_data = [
                (idx, a) for idx, a in enumerate(batch_audios) if a is not None
            ]
            if not valid_data:
                continue

            valid_indices, valid_audios = zip(*valid_data)
            padded = pad_sequence(valid_audios, batch_first=True).to(self.get_device())

            with torch.no_grad():
                batch_embeds = self.classifier.encode_batch(padded).squeeze(1)
                batch_embeds = torch.nn.functional.normalize(batch_embeds, dim=1)

            for idx, orig_idx in enumerate(valid_indices):
                # Store on CPU to avoid holding GPU memory across groups
                embeddings[batch_paths[orig_idx]] = batch_embeds[idx].cpu()
    ...
    def calculate_batch_optimized(self, pairs: list[tuple[Path, Path]]) -> list[float]:
        ...
        try:
            ...
            embeddings = self.extract_embeddings(all_paths)

            self.logger.info(f"Calculating similarities for {len(pairs)} pairs")
            for idx, (ref_path, syn_path) in tqdm(
                enumerate(pairs),
                total=len(pairs),
            ...
            self.logger.error(
                f"SIM batch optimization failed: {e}. Falling back to individual processing."
            )
